...
export default function IPOListing() {
    ...
    const fetchTables = useCallback(async () => {
        ...
        try {
            ...
            if (result.success && Array.isArray(result.data)) {
                ...
                const initialSortConfigs: Record<string, { key: string; direction: "asc" | "desc" } | null> = {}
    ...
    const exportData = (tableId: string) => {
        ...
        const csvContent = [
            ...
            ...data.map((row) => table.columns.map((col) => String(row[col.name] || "")).join(",")),
        ].join("\n")
        ...
        tables.forEach((table) => {
            ...
            if (searchQuery && table.settings.searchable) {
                data = data.filter((row) =>
                    Object.entries(row).some(([, value]) => String(value).toLowerCase().includes(searchQuery.toLowerCase())),
                )
            }
    ...
    if (tables.length === 0) {
        return (
            <div className="p-4 sm:p-6 lg:p-8 flex flex-col items-center justify-center min-h-[400px]">
                ...
                <p className="text-muted-foreground mb-4">No data tables are currently available.</p>
        ...
        <div className="p-4 sm:p-6 lg:p-8 space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">View and manage all available IPO data tables</p>
                </div>
            ...
            {tables.length > 1 ? (
                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                    <TabsList className="mb-4 w-full sm:w-auto overflow-x-auto flex-nowrap">
                        {tables.map((table) => (
                            <TabsTrigger key={table._id} value={table._id} className="whitespace-nowrap">
                                {table.tableName}
                            </TabsTrigger>
                        ))}
                    </TabsList>
                    ...
                    {tables.map((table) => (
                        <TabsContent key={table._id} value={table._id} className="mt-0">
                            <TableDisplay
                                ...
                                onSearch={(value) => setSearchQueries((prev) => ({ ...prev, [table._id]: value }))}
                                onSort={(key) => {
                                ...
                                onPageChange={(page) => setCurrentPages((prev) => ({ ...prev, [table._id]: page }))}
                        ...
                        <div className="flex items-center gap-2">
                                        ...
                                        <div className="mt-6 space-y-6">
                                                ...
                                                <div key={column.name} className="space-y-2">
                                                    ...
                                                    {column.options ? (
                                                        <Select
                                                            ...
                                                            onValueChange={(value) => onFilter(column.name, value)}
                                                        >
                                                            <SelectTrigger>
                                                                <SelectValue placeholder={`Filter by ${column.name}`} />
                                                            </SelectTrigger>
                                                            <SelectContent>
                                                        ...
                                                        <Input
                                                            ...
                                                            value={tableFilters[column.name] || ""}
                                                            onChange={(e) => onFilter(column.name, e.target.value)}
                                                        />
                                                    )}
                                                </div>
                                            ))}
                                            ...
                                            {activeFiltersCount > 0 && (
                                                <Button
                                                    ...
                                                    onClick={() => table.columns.forEach((col) => onFilter(col.name, ""))}
                                                >
                                                    Clear All Filters
                                                </Button>
                                            )}
                            ...
                            {table.settings.pagination && (
                                <Select value={String(currentPage)} onValueChange={(value) => onPageChange(Number(value))}>
                                    ...
                                    <SelectContent>
                                        {Array.from({
                                            length: Math.ceil(processedData.length / table.settings.itemsPerPage) || 1,
                                        }).map((_, i) => (
                                    ...
                                    <TableHead key={column.name} className="whitespace-nowrap">
                                        <div className="flex items-center gap-2">
                                            ...
                                            {table.settings.sortable && (
                                                ...
                                                >
                                                    <ArrowUpDown
                                                        className={`h-3 w-3 ${sortConfig?.key === column.name ? "text-primary" : "text-muted-foreground"
                                                            }`}
                                                    />
                                                </Button>
                                            )}
                                        ...
                                        {searchQuery || activeFiltersCount > 0 ? (
                                            <div className="flex flex-col items-center justify-center">
                                                ...
                                                <Button
                                                    ...
                                                    onClick={() => {
                                                        onSearch("")
                                                        table.columns.forEach((col) => onFilter(col.name, ""))
                                                    }}
                                                    className="mt-2"
                                                >
                                                    Clear all filters
                ...
                {table.settings.pagination && processedData.length > 0 && (
                    <div className="flex flex-col sm:flex-row items-center justify-between border-t p-4 gap-4">
                        <div className="text-sm text-muted-foreground order-2 sm:order-1">
                            Showing {Math.min((currentPage - 1) * table.settings.itemsPerPage + 1, processedData.length)} to{" "}
                            {Math.min(currentPage * table.settings.itemsPerPage, processedData.length)} of {processedData.length}{" "}
                            results
                        </div>
                        <div className="flex items-center gap-2 order-1 sm:order-2">
                            ...
                            <Button
                                ...
                                disabled={currentPage * table.settings.itemsPerPage >= processedData.length}
                            >
                                Next
                            </Button>
                        </div>
    ...
    switch (type.toLowerCase()) {
        ...
        case "percentage":
            return typeof value === "number"
                ...
                : value
        case "status":
            return <Badge variant={getStatusVariant(String(value))}>{String(value)}</Badge>
        default:
            return String(value)
    }
}
...
function getStatusVariant(status: string): "default" | "secondary" | "destructive" | "outline" {
    ...
    if (
        ...
        return "default"
    } else if (lowercaseStatus.includes("pending") || lowercaseStatus.includes("processing")) {
        return "secondary"
    } else if (
